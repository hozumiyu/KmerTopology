...
from algorithm.kmers_Laplacian import KmersLaplacian
...
outpath = args.name + '_Laplacian/' ; makeFolder(outpath)
spectrum_file = outpath + args.name + '_%d_spectrum.txt'%args.kmers_size
betti0_file = outpath + args.name + '_%d_betti0_feature.npy'%args.kmers_size
eigmin_file = outpath + args.name + '_%d_eigmin_feature.npy'%args.kmers_size

start = time.time()

myKmerLap = KmersLaplacian(kmers_size = args.kmers_size, steps = steps)
